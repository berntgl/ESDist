esd_plot <- function(df, es, es_type, method = "quads", bin_width = 0.05) {
  ...
  if (method == "quads") {
    ...
    cdq1 <- quantile(es_col, prob = 0.1665)
    cdq1_label <- "16.65%"
    cdq2 <- quantile(es_col, prob = 0.50)
    cdq2_label <- "50%"
    cdq3 <- quantile(es_col, prob = 0.8335)
    cdq3_label <- "83.35%"
  } else {
  ...
  ggplot(data = df, aes(es_col)) +
    ...
    geom_vline(aes(xintercept = cdq2), color = "#E5A4A9", linetype = "dashed", size = 1) +
    annotate("text", x = (cdq2 + 0.05), y = 2, color = "#E5A4A9", label = cdq2_label, angle = 90) +
    geom_vline(aes(xintercept = cdq3), color = "#E5A4A9", linetype = "dashed", size = 1) +
    annotate("text", x = (cdq3 + 0.05), y = 2, color = "#E5A4A9", label = cdq3_label, angle = 90) +
